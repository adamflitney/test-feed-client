...
class UserFeed extends Component {
    ...
    likes = this.state.notifications.commentsByPost.map(notification => {
            if (notification.items.length > 2) {
                let remaining = notification.items.length - 2;
                return (
                    <div>
                        <span className="names">{notification.items[0].user.name}</span>
                    </div>
                );
            } else {
            ...
            <div className="container" ref={this.container}>
                ...
                {this.state.isOpen &&
                    <div className="feed-list">
                        {
                            this.state.notifications.commentsByPost.map(notification => {
                                return <div>{notification.title}</div>
                            })}
                        {this.likes}
                    </div>
